...
export default function ProjectApplyList(){
  const { res, loading } = useSelectProjectApplication('1', '1');
  ...
  return (
    <div className='main-page align-stretch' style={{ minHeight: 'calc(100vh - 7rem)' }}>
      ...
      <Paper className='mypage-box flex-col flex-grow' elevation={4}>
        {loading && <p style={{ padding: '1rem' }}>로딩 중...</p>}
        ...
        >
          ...
          >
            <SwiperSlide>
              <ApplicantCard
                applicationDate={application?.applicationDate ?? '2025.12.03'}
                userName={application?.userName ?? '홍길동'}
                userEmail={application?.userEmail ?? 'email@gmail.com'}
                mannerTemperature={application?.mannerTemperature ?? '36.5'}
              />
            </SwiperSlide>
            <SwiperSlide>
              <ApplicantCard 
