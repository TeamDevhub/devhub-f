      ...
      <div className="flex-col" style={{ gap: '0.8rem' }}>
        ...
        <Paper className="auth-box flex-col" elevation={4}>
          ...
          <div className="field-box flex-col">
            ...
            <div className="field-content flex-col">
              <CustomTextfield type="password" placeholder="특수문자, 숫자 포함 10자 이상" value={password} onChange={(e) => setPassword(e.target.value)} />
              <CustomTextfield type="password" placeholder="비밀번호 확인" value={passwordConfirm} onChange={(e) => setPasswordConfirm(e.target.value)} />
            </div>
          </div>
          ...
          <div className="field-box flex-col">
            ...
            <div className="field-content flex-col" style={{ gap: '0.5rem' }}>
              <CustomTextfield placeholder="닉네임" value={nickname} onChange={(e) => setNickname(e.target.value)} />
              <span className="help-text">다른 사용자에게 표시되는 이름입니다</span>

              <CustomTextfield type="textarea" rows={2} placeholder="자신을 소개해 주세요." value={intro} onChange={(e) => setIntro(e.target.value)} />
            </div>
          </div>
            ...
            <div className="field-content flex-col" style={{ gap: '0.5rem' }}>
              <div className="chip-box w-100 align-center flex-wrap">
                {positionOptions.map((pos) => (
                  <Chip key={pos} label={pos} color={positions.includes(pos) ? 'primary' : 'default'} clickable onClick={() => togglePosition(pos)} />
                ))}
              </div>
          ...
          <div className="field-box2 flex-col">
            ...
            <div className="field-content flex-col" style={{ gap: '0.5rem' }}>
              <div className="content-box align-stretch">
                <div className="chip-box align-center flex-wrap" style={{ flex: 1 }}>
                  {skills.length === 0 ? <span className="help-text">선택된 스킬이 없습니다</span> : skills.map((skill) => <Chip key={skill} label={skill} color="primary" />)}
                </div>
                <Button size="large" variant="contained" color="primary" onClick={openSkillSelectDialog}>
                  +
                </Button>
              </div>
          ...
          <Button size="large" variant="contained" color="primary" endIcon={<ArrowForwardIos sx={{ fontSize: '1.2rem !important' }} />} fullWidth onClick={handleSignup} disabled={loading}>
            {loading ? '가입 중...' : '회원가입'}
          </Button>
        </Paper>

        <Dialog open={openSkillDialog} onClose={() => setOpenSkillDialog(false)} fullWidth maxWidth="sm">
          ...
          >
            <div className="chip-box align-center flex-wrap">
              {skillOptions.map((skill) => (
                <Chip key={skill} label={skill} color={tempSkills.includes(skill) ? 'primary' : 'default'} clickable onClick={() => toggleTempSkill(skill)} />
              ))}
            </div>
          </DialogContent>
